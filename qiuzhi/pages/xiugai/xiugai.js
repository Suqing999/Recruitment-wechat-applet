...
Page({
  ...
  getxingxi(){
    let that = this;
    wx.request({
      url: 'http://localhost:8080/zhaoping/getxingxi',
      method: 'GET',
      header: {
      ...
      success(res) {
        console.log(res);
        that.setData({
          jobtitle:res.data.qyx,
          nianfeng: res.data.qdate,
          name:res.data.qname,
          email:res.data.qmail,
          address:res.data.qjzd,
          tel:res.data.qphone,
          content:res.data.qjs
        })
      },
      fail: function (res) {
        console.log("--------fail--------");
  ...
  formSubmit: function (e) {
    ...
    wx.request({
      url: 'http://localhost:8080/zhaoping/toudijianli2',
      method: 'GET',
      data: {
        qid: app.appdata.userinfo.qid,
